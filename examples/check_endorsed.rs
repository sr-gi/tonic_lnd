// This example connects to LND and checks if the htlcs in an invoice are endorsed
//
// The program accepts four arguments: address, cert file, macaroon file, payment hash
// The address must start with `https://`!
//
// Example run: `cargo run --features=lightningrpc --example check_endorsed <address> <tls.cert> <file.macaroon> <payment_hash>`
...
async fn main() {
    ...
    let invoices = client
        ...
        .list_invoices(fedimint_tonic_lnd::lnrpc::ListInvoiceRequest {pending_only: false, index_offset: 0, num_max_invoices: 0, reversed: false})
        .await
